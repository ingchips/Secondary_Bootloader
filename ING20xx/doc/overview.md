...
```
ING20xx/                          # 工程根目录
├── 1_secondary_bootloader/       # 二级 bootloader 工程，仅演示固件搬运的功能，纯裸机实现，不依赖 platform
├── 1_secondary_bootloader_usb/   # 二级 bootloader 工程，集成 USB 在线升级、固件搬运的功能，纯裸机实现，不依赖 platform
...
* **PLATFORM**：原厂维护协议栈 PATCH 程序，部分开放，用户尽量不修改，必要时可做少量修改。**起始地址 `__PLATFORM_ADDR` 可以自定义放置到二级 Boot 之后，最好是贴着二级 Boot 放置**，节省空间，比如二级 Boot 的大小为 8KB，则 PLATFORM 的起始地址可以为 0x02004000。**`__PLATFORM_ADDR` 由二级 Boot 决定，固化后不可改变。**
* **APP**: 用户维护应用程序。**起始地址 `__APP_LOAD_ADD` 可以自定义放置到 PLATFORM 之后，`__APP_LOAD_ADD` 也由二级 Boot 决定，固化后不可改变。随着 SDK 的更新，PATCH 可能增大，platform.bin 的大小也会变大，应预留足够的空间给 platform(patch)。如果预留空间不够，后期更新  platform(patch) 会覆盖到 APP 不好解决。** 例如 `1_secondary_bootloader` 工程的 `__APP_LOAD_ADD` 设置为 `0x0200A000`，预留了 `24KB` 给 platform(patch) 使用。

`1_secondary_bootloader` 工程 Flash 分布如下：
...
`1_secondary_bootloader_usb` 工程 Flash 分布如下：

| 地址 | 大小 | 说明 |
| ---- | ---- | ---- |
| 0x2002000 | 0x6000 | secondary_bootloader |
| 0x2008000 | 0x6000 | platform_companion |
| 0x200E000 | \ | APP |
