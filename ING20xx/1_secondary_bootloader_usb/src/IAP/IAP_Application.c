...
#define IAP_APP_ERROR(...)	printf(__VA_ARGS__)
#else
#define IAP_APP_ERROR(...)      
#endif

#if USER_IAP_APP_DEBUG_LOG_EN
#define IAP_APP_DEBUG(...)	printf(__VA_ARGS__)
#else
...
static const char bHardware[]    = "V0.0.1";
static const char bSoftware[]    = "V0.0.1";
...
static void IAP_DoUpdate_Delay_Timeout_Callback(void){
...
static IAP_APP_ErrCode_t IAP_CMD_Reboot_handler(uint8_t * payload, uint16_t length){
	...
	app_timer_start(IAP_Reboot_Delay_Timeout_Callback, *delay_ms);
...
static IAP_APP_ErrCode_t IAP_CMD_SwitchApp_handler(uint8_t * payload, uint16_t length){
	...
	app_timer_start(IAP_DoUpdate_Delay_Timeout_Callback, *delay_ms);
